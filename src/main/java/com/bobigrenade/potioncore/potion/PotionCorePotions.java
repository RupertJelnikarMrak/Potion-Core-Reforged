...
import com.bobigrenade.potioncore.effect.PotionCoreEffects;
...
public class PotionCorePotions {
    ...
    public static final RegistryObject<Potion> FREEZE_POTION = POTIONS.register(
        "freeze_potion",
        () -> new Potion(new MobEffectInstance(PotionCoreEffects.FREEZE.get(), 200, 0))
    );

    // * Cure potion
    public static final RegistryObject<Potion> CURE_POTION = POTIONS.register(
        "cure_potion",
        () -> new Potion(new MobEffectInstance(PotionCoreEffects.CURE.get(), 0, 0))
    );
